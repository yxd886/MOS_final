 ...
 */
#define AHO_PATTERN_FILE "aho-corasick/snort_dfa_pattern"
...
*aho_get_strings(const char *pattern_file, int *num_patterns)
{
    ...
    if(fscanf(pattern_fp, "%d", num_patterns)==0){
        exit(-1);
    }
    ...
    if(getline(&first_newline, &buf_size, pattern_fp)==0){
        printf("no line\n");
    }
...
*aho_get_patterns(const char *pattern_file, int *num_patterns)
{
    ...
    for(i = 0; i < *num_patterns; i++) {
        ...
        if(fscanf(pattern_fp, "%d", &dfa_id)==0){
           exit(-1);
        }
        ...
        for(j = 0; j < len; j++) {
            int cur_byte;
            if(fscanf(pattern_fp, "%d", &cur_byte)==0){
                exit(-1);
            }
